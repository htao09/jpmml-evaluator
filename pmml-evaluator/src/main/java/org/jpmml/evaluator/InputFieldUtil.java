...
public class InputFieldUtil {
	...
	static public boolean isDefault(Field<?> field, MiningField miningField) {
	...
	static public FieldValue prepareInputValue(Field<?> field, MiningField miningField, Object value) {
	...
	static public FieldValue prepareResidualInputValue(DataField dataField, MiningField miningField, Object value) {
		return prepareInputValue(dataField, miningField, value);
	}

	static private ScalarValue prepareScalarInputValue(InputTypeInfo typeInfo, Object value) {
	...
	static private ScalarValue performValidValueTreatment(InputTypeInfo typeInfo, ScalarValue value) {
	...
	static private ScalarValue performInvalidValueTreatment(InputTypeInfo typeInfo, Object value) {
		MiningField miningField = typeInfo.getMiningField();
		...
		case RETURN_INVALID:
			Object invalidValueReplacement = miningField.getInvalidValueReplacement();
			if (invalidValueReplacement != null) {
				throw new MisplacedAttributeException(miningField, PMMLAttributes.MININGFIELD_INVALIDVALUEREPLACEMENT,
		...
		case RETURN_INVALID:
			...
			throw new InvalidResultException("Field " + PMMLException.formatKey(field.getName())
					+ " cannot accept user input value " + PMMLException.formatValue(value), miningField);
		case AS_IS:
	...
	static private ScalarValue performMissingValueTreatment(InputTypeInfo typeInfo) {
		MiningField miningField = typeInfo.getMiningField();
		...
		case RETURN_INVALID:
			...
			Object missingValueReplacement = miningField.getMissingValueReplacement();
			if (missingValueReplacement != null) {
				throw new MisplacedAttributeException(miningField, PMMLAttributes.MININGFIELD_MISSINGVALUEREPLACEMENT,
						missingValueReplacement);
			}

			throw new InvalidResultException(
					"Field " + PMMLException.formatKey(field.getName()) + " requires user input value", miningField);
		default:
			throw new UnsupportedAttributeException(miningField, missingValueTreatmentMethod);
		}
	}

	static private Integer getStatus(InputTypeInfo typeInfo, Object value, boolean compatible) {
		Field<?> field = typeInfo.getField();

		values: if (field instanceof HasDiscreteDomain) {
			...
			// "If a field contains at least one Value element where the value of property
			// is valid, then the set of Value elements completely defines the set of valid
			// values"
			if (validIndex > 0) {
				return FieldValue.STATUS_UNKNOWN_INVALID;
			}
		...
		intervals: if (field instanceof HasContinuousDomain) {
			...
			// "If intervals are present, then a value that is outside the intervals is
			// considered invalid"
	...
	static private ScalarValue createInvalidInputValue(InputTypeInfo typeInfo, Object value) {
		MiningField miningField = typeInfo.getMiningField();
	...
	static private ScalarValue createMissingInputValue(InputTypeInfo typeInfo) {
		MiningField miningField = typeInfo.getMiningField();
	...
	static private FieldValue createInputValue(TypeInfo typeInfo, Object value) {
	...
	static private InputTypeInfo getTypeInfo(Field<?> field, MiningField miningField) {
		InputTypeInfo typeInfo = new InputTypeInfo() {
			...
			public DataType getDataType() {
				...
				if (dataType == null) {
					throw new MissingAttributeException(MissingAttributeException
							.formatMessage(XPathUtil.formatElement(field.getClass()) + "@dataType"), field);
				}
			...
			public OpType getOpType() {
				...
				if (opType == null) {
					throw new MissingAttributeException(MissingAttributeException
							.formatMessage(XPathUtil.formatElement(field.getClass()) + "@optype"), field);
				}
